...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function init_template() {
        ...
        darkModeSwitch.forEach(el => el.addEventListener('click', e => {
            if (document.body.className == "theme-light") { removeTransitions();
                activateDarkMode(); } else if (document.body.className == "theme-dark") { removeTransitions();
                activateLightMode(); }
            setTimeout(function() { addTransitions(); }, 350);
        }));

        //Set Color Based on Remembered Preference.
        if (localStorage.getItem(pwaName + '-Theme') == "dark-mode") { for (let i = 0; i < toggleDark.length; i++) { toggleDark[i].checked = "checked" };
        ...
        if (locationBut.length) {
            ...
            function geoLocate() {
                ...
                if (!navigator.geolocation) { locationCoordinates.textContent = 'Geolocation is not supported by your browser'; } else { locationCoordinates.textContent = 'Locating';
                    navigator.geolocation.getCurrentPosition(success, error); }
            }
            var getLocation = document.getElementsByClassName('get-location')[0]
            if (typeof(getLocation) != 'undefined' && getLocation != null) {
                getLocation.addEventListener('click', function() { this.classList.add('disabled');
                    geoLocate(); })
            }
        }
        ...
        */
        ...
        if (shareCheck.length) {
            ...
            document.querySelectorAll('.shareToTwitter').forEach(x => x.setAttribute("href", "https://twitter.com/home?status=" + share_link));
        ...
        function offlinePage() {
            ...
            anchorsDisabled.forEach(function(e) {
                ...
                if (hrefs.match(/.html/)) { e.classList.add('show-offline');
        ...
        function onlinePage() {
            ...
            anchorsEnabled.forEach(function(e) {
                ...
                if (hrefs.match(/.html/)) { e.setAttribute('href', hrefs);
